...
import  {WelcomeContent, SearchDoneContent, SearchErrorContent, MovieDetailContent} from './components/Content'
  ...
  render() { //어떤 html을 그릴 것인가를 결정하는 함수 (하위의 컴퍼넌트들이 바뀌면 다시 render함수가 실행됨 >> html이 업데이트 됨.)
    ...
    return ( //html파일로 return
      <div id="wrapper">
        ...
        {mode === 'welcome' ? <WelcomeContent /> :
          mode === 'show' ? <MovieDetailContent movie={this.state.show}/> :
          mode === 'search' ? <SearchDoneContent /> :
        mode === 'error' ? <SearchErrorContent /> : null
        }
      </div>
    );
  }
